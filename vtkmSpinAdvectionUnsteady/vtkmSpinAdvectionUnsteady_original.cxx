...
#pragma omp parallel for   // Para entrada/salida con openMP. Independiente de cómo se resuelvan las trayectorias. Esto es para paralelización híbrida
  ...
  for (size_t i = 0; i < fileNames.size() - 1; i++) {
    ...
    std::cout << "DS1: " << fileNames[i] << ", Timestamp: " << timestamps[i]
              << std::endl;
    std::cout << "DS2: " << fileNames[i + 1]
              << ", Timestamp: " << timestamps[i + 1] << std::endl;
    ...
    std::cout << "Time spent per spin advection computation: "
              << timer.GetElapsedTime() << " seconds" << std::endl;
    ...
    timer.Stop();
    std::cout << "Time spent writing output to disk: " << timer.GetElapsedTime()
              << " seconds" << std::endl;
    ...
    timer.Stop();
    std::cout << "Time needed for reseeding " << timer.GetElapsedTime()
              << std::endl;
    totalIOTime += timer.GetElapsedTime();
    timer.Reset();
